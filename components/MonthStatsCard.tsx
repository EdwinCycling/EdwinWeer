...
export const MonthStatsCard: React.FC<Props> = ({ location, settings, onNavigate, weatherData }) => {
            ...
            try {
                ...
                if (result && result.daily) {
                    ...
                    if (daily.temperature_2m_mean) {
                        avgTemp = daily.temperature_2m_mean.reduce((a: number, b: number) => a + b, 0) / len;
                    } else {
                    ...
                    const avgMax = daily.temperature_2m_max ? daily.temperature_2m_max.reduce((a: number, b: number) => a + b, 0) / len : 0;
                    const avgMin = daily.temperature_2m_min ? daily.temperature_2m_min.reduce((a: number, b: number) => a + b, 0) / len : 0;
                    const rainSum = daily.precipitation_sum ? daily.precipitation_sum.reduce((a: number, b: number) => a + b, 0) : 0;
                    
                    // Rain days > 1mm (as before, but usually > 0.1 or 1.0 is used)
                    const rainDays = daily.precipitation_sum ? daily.precipitation_sum.filter((p: number) => p > 1).length : 0;
                    
                    // New: Rain days >= 2mm (Natte dagen)
                    const rainDays2mm = daily.precipitation_sum ? daily.precipitation_sum.filter((p: number) => p >= 2).length : 0;
                    
                    // Sun days: > 50% of daylight duration
                    let sunDays = 0;
                    if (daily.sunshine_duration && daily.daylight_duration) {
                        daily.sunshine_duration.forEach((sun: number, i: number) => {
                            const daylight = daily.daylight_duration[i];
                            if (daylight > 0 && (sun / daylight) >= 0.5) {
                                sunDays++;
                            }
                        });
                    } else if (daily.sunshine_duration) {
                        // Fallback if daylight_duration is missing (should not happen with new fetch)
                         sunDays = daily.sunshine_duration.filter((d: number) => d > 14400).length;
                    }

                    // Calculate Avg Wind
                    let avgWind = 0;
                    if (daily.wind_speed_10m_mean) {
                        avgWind = daily.wind_speed_10m_mean.reduce((a: number, b: number) => a + b, 0) / len;
                    } else if (daily.wind_speed_10m_max) {
                        avgWind = daily.wind_speed_10m_max.reduce((a: number, b: number) => a + b, 0) / len;
                    }
        ...
        <>
            ...
            >
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {/* Row 1: Gem. Temp, Gem. Max, Gem. Min */}
                    <div className="flex flex-col gap-1 p-3 rounded-xl bg-bg-page/50 border border-border-color/50">
                    ...
                    {/* Row 2: Hoogste Max, Laagste Min, Neerslag Totaal */}
                    <div className="flex flex-col gap-1 p-3 rounded-xl bg-bg-page/50 border border-border-color/50">
                    ...
                    {/* Empty placeholder if needed or just let grid flow */}

                    {/* Row 3: Natte Dagen, Zondagen, Mini Charts */}
                     <div className="flex flex-col gap-1 p-3 rounded-xl bg-bg-page/50 border border-border-color/50">
