        ...
        <footer className="py-12 bg-slate-950 text-white border-t border-white/10 relative z-10 pb-24 md:pb-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <p className="text-white/40 text-sm">
                  © {new Date().getFullYear()} Baro Weather. {t('landing.rights_reserved')}
              </p>
              <div className="flex gap-6 text-sm text-white/40">
                  <button onClick={() => onNavigate(ViewState.FAQ)} className="hover:text-white transition-colors">{t('landing.nav_faq')}</button>
                  <button onClick={handleLogin} className="hover:text-white transition-colors">{t('landing.nav_login')}</button>
              </div>
          </div>
        </footer>

        {/* Mobile Bottom Nav */}
        <div className="fixed bottom-0 left-0 right-0 bg-black/60 backdrop-blur-xl border-t border-white/10 z-[100] md:hidden">
          <div className="flex justify-around items-center p-4 pb-6" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
            <button 
              onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
              className="flex flex-col items-center gap-1 text-white/60 hover:text-white transition-colors"
            >
              <Icon name="home" className="text-xl" />
              <span className="text-[10px] font-medium uppercase">{t('landing.nav_home')}</span>
            </button>
            <button 
              onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
              className="flex flex-col items-center gap-1 text-white/60 hover:text-white transition-colors"
            >
              ...
              <span className="text-[10px] font-medium uppercase">{t('landing.nav_features')}</span>
            </button>
            <button 
              onClick={() => onNavigate(ViewState.FAQ)}
              className="flex flex-col items-center gap-1 text-white/60 hover:text-white transition-colors"
            >
              ...
              <span className="text-[10px] font-medium uppercase">{t('landing.nav_faq')}</span>
            </button>
            <button 
              onClick={handleLogin}
              className="flex flex-col items-center gap-1 text-white/60 hover:text-white transition-colors"
            >
              ...
              <span className="text-[10px] font-medium uppercase">{t('landing.nav_login')}</span>
            </button>
          </div>
        </div>
      </div>
